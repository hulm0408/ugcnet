...
export async function generateMetadata({ params }: SubjectPageProps): Promise<Metadata> {
  ...
  const subjectDb = await prisma.subject.findFirst({ where: { slug } });
  if (!subjectDb) {
    return { title: 'Subject Not Found' };
  }

  const config = getSubjectConfig(slug, subjectDb);
  const totalQuestions = await prisma.question.count({
    where: { content_status: 'PUBLISHED', subject_id: subjectDb.id },
  });

  return {
    title: `UGC NET ${config.name} (${config.nativeName}) — Syllabus, Solved PYQs & CBT Mock Tests`,
    description: `Prepare for UGC NET ${config.name} (Subject Code ${config.code}) with authentic question papers, official 10 units, mistake tracking, and CBT simulation.`,
    alternates: { canonical: `/subject/${slug}` },
    openGraph: {
      title: `UGC NET / JRF ${config.name} (${config.nativeName}) Preparation Platform`,
      description: `Complete NTA 10-unit syllabus, previous year question papers, and CBT mock tests for ${config.name}.`,
    },
  };
}
...
export default async function DedicatedSubjectLandingPage({ params }: SubjectPageProps) {
  ...
  return (
    <div className="flex-1 overflow-hidden bg-white text-stone-900">
          ...
          <div className="grid lg:grid-cols-12 gap-10 lg:gap-8 items-center">
            ...
            <div className="lg:col-span-7 space-y-6 text-left">
              <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-950/80 border border-emerald-500/30 text-emerald-400 text-xs font-mono font-bold tracking-wider uppercase">
                ...
                UGC NET / JRF {config.name.toUpperCase()} (CODE {config.code}) • DEDICATED PREP
              </div>
            ...
            {/* Right Column: Tailored SVG Artwork */}
      ...
      {/* ── 5. MEMORY ENGINE DIFFERENTIATOR ── */}
